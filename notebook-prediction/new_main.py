...
if __name__ == "__main__":
    ...
    class genDataset(Dataset):
        def __init__(self):
            ...
            self.embed_arr = np.load("train_split4/{}_embed_list_total.npy".format(data_type))
    ...
    if mode == 'embed_doc2vec':
        ...
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            ...
            embed = model.infer_vector(tokenize_code(source))
            embed_list.append(embed)
    ...
    if mode == "111":
        df = pd.read_csv("{}_loc_dataset.csv".format(data_type))
        print(np.unique(df["competition"]))
    ...
    if mode == 'train_gen':
        ...
        data, lengths = next(iter(valid_loader))
        ...
        with torch.no_grad():
            ...
            for i in trange(len(test_dataset)):
                ...
                for idx in range(2, length):
                    ...
                    rank_list.append(np.where(result == start + idx)[0][0])
                    # actual_meta = df.loc[start + idx]
                    # print(actual_meta)
                # print(rank_list)
        rank_list = np.array(rank_list)
        print(np.mean(rank_list))
        np.save('./{}_rank_list'.format(data_type), rank_list)
